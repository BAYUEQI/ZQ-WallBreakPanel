...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.wbp.get("proxySettings", { type: 'json' });
        warpConfigs = await env.wbp.get('warpConfigs', { type: 'json' });
    } catch (error) {
        console.log(error);
...
export async function updateDataset(request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.wbp.get("proxySettings", { type: 'json' });
        } catch (error) {
            console.log(error);
    ...
    try {
        await env.wbp.put("proxySettings", JSON.stringify(settings));
    } catch (error) {
        console.log(error);
