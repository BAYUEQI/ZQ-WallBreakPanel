...
async function getSettings(request, env) {
    try {
        const isPassSet = await env.wbp.get('pwd') ? true : false;
        const auth = await Authenticate(request, env);
...
async function getWarpConfigs(request, env) {
    ...
    try {
        warpEndpoints.forEach((endpoint, index) => {
            zip.file(`${atob('V0JQ')}-Warp-${index + 1}.conf`, trimLines(
                `[Interface]
        ...
        return new Response(arrayBuffer, {
            headers: {
                ...
                "Content-Disposition": `attachment; filename="${atob('V0JQ')}-Warp-${isPro ? "Pro-" : ""}configs.zip"`,
            },
        });
    } catch (error) {
...
async function renderPanel(request, env) {
    const pwd = await env.wbp.get('pwd');
    if (pwd) {
        const auth = await Authenticate(request, env);
