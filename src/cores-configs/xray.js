...
async function buildXrayBestPingConfig(totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? `💦 ${atob('V0JQ')} F - Best Ping 💥` : `💦 ${atob('V0JQ')} - Best Ping 💥`;
...
async function buildXrayBestFragmentConfig(hostName, chainProxy, outbound) {
    ...
    const config = await buildXrayConfig(`💦 ${atob('V0JQ')} F - Best Fragment 😎`, true, chainProxy, false, false, true, false, [], hostName);
...
async function buildXrayWorkerLessConfig() {
    const cfDnsConfig = await buildXrayConfig(`💦 ${atob('V0JQ')} F - WorkerLess - 1 ⭐`, false, false, false, false, true, true, [], false, "cloudflare-dns.com", ["cloudflare.com"]);
    const googleDnsConfig = await buildXrayConfig(`💦 ${atob('V0JQ')} F - WorkerLess - 2 ⭐`, false, false, false, false, true, true, [], false, "dns.google", ["8.8.8.8", "8.8.4.4"]);
    ...
    if (settings.outProxy) {
        try {
            ...
            chainProxy = undefined;
                const proxySettings = await env.wbp.get("proxySettings", { type: 'json' });
    await env.wbp.put("proxySettings", JSON.stringify({
                ...proxySettings,
                outProxy: '',
                outProxyParams: {}
            }));
